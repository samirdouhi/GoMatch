...
namespace BusinessService.Services
{
    public class ServicePhotoCommerce : IServicePhotoCommerce
    {
        ...
        public async Task<PhotoCommerceReponseDto?> AjouterAsync(
            Guid commerceId, Guid utilisateurId, IFormFile fichier)
        {
            if (fichier.Length == 0 || fichier.Length > TailleMaxFichier) return null;

            var extension = Path.GetExtension(fichier.FileName).ToLowerInvariant();
            if (!ExtensionsAutorisees.Contains(extension)) return null;

            var commerce = await _commerceRepo.ObtenirParIdAsync(commerceId);
            if (commerce == null || commerce.ProprietaireUtilisateurId != utilisateurId) return null;

            var nbExistant = await _photoRepo.CompterParCommerceAsync(commerceId);
            if (nbExistant >= MaxPhotosParCommerce) return null;
        ...
        {
            var commerce = await _commerceRepo.ObtenirParIdAsync(commerceId);
            if (commerce == null || commerce.ProprietaireUtilisateurId != utilisateurId) return false;

            var photo = await _photoRepo.ObtenirParIdAsync(photoId);
            if (photo == null || photo.CommerceId != commerceId) return false;
        ...
        {
            var photo = await _photoRepo.ObtenirParIdAsync(photoId);
            if (photo == null || !File.Exists(photo.CheminFichier)) return null;
