...
// Marqueur dégradé orange GoMatch
...
async function reverseGeocode(lat: number, lng: number): Promise<string> {
  try {
    const res = await fetch(
      ...
      { headers: { "Accept-Language": "fr", "User-Agent": "GoMatch-App/1.0" } }
    );
    if (!res.ok) return "";
    ...
    <div className="flex flex-col gap-3">
      ...
      >
        <MapContainer
          ...
          zoomControl={false}
        >
          {/* Tiles OpenStreetMap — fiables et sans clé API */}
          <TileLayer
                ...
                <div style={{ minWidth: 190, padding: "2px 0" }}>
                  ...
                  {value?.address && (
                    <p style={{ fontSize: 12, color: "#555", lineHeight: 1.5 }}>
                      {value.address.length > 100 ? value.address.slice(0, 100) + "…" : value.address}
                    </p>
                  )}
                  <p style={{ fontFamily: "monospace", fontSize: 11, color: "#888", marginTop: 8 }}>
        ...
        {/* Zoom custom */}
        <div className="absolute left-3 top-3 z-[500] flex flex-col overflow-hidden rounded-xl border border-zinc-300/60 bg-white shadow-md">
          <button
            type="button"
            onClick={() => document.querySelector<HTMLElement>(".leaflet-control-zoom-in")?.click()}
            className="flex h-8 w-8 items-center justify-center text-base text-zinc-700 hover:bg-zinc-100"
          >+</button>
          <div className="h-px bg-zinc-200" />
          <button
            type="button"
            onClick={() => document.querySelector<HTMLElement>(".leaflet-control-zoom-out")?.click()}
            className="flex h-8 w-8 items-center justify-center text-base text-zinc-700 hover:bg-zinc-100"
          >−</button>
        </div>
        ...
        <button
          ...
          disabled={gpsLoading}
          className="absolute bottom-4 right-4 z-[500] flex items-center gap-2 rounded-xl border border-zinc-300/60 bg-white px-3 py-2 text-xs font-semibold text-zinc-800 shadow-md transition hover:bg-zinc-50 disabled:opacity-60"
        >
          {gpsLoading
      ...
      {value ? (
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-xl border border-white/10 bg-black/20 px-4 py-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">Latitude</p>
            <p className="mt-1 font-mono text-sm text-orange-300">{value.lat.toFixed(6)}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-black/20 px-4 py-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">Longitude</p>
