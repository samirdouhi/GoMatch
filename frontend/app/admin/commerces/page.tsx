...
import {
  ...
  MapPin, Tag, Search, RefreshCw, ChevronDown, ChevronUp,
  User, Mail, Calendar, Filter,
} from "lucide-react";
import {
...
function StatutBadge({ statut }: { statut: string }) {
  const MAP: Record<string, { icon: React.ElementType; label: string; cls: string }> = {
    ...
    Approuve:  { icon: CheckCircle2,  label: "Approuvé",    cls: "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" },
      ...
      <div className="w-full max-w-lg rounded-3xl border border-red-500/20 bg-[#0d0f14] p-6 shadow-2xl">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-0.5">{commerce.nom}</p>
          </div>
        </div>
        ...
        <textarea
          ...
          rows={4}
          placeholder="Expliquez pourquoi la demande est refusée (ex. informations incomplètes, adresse invalide…)"
          className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:text-zinc-600 focus:border-red-500/50 focus:outline-none focus:ring-1 focus:ring-red-500/30 resize-none"
        />
        <p className="mt-1 text-[11px] text-zinc-600">{raison.length}/500 caractères</p>
...
function CommerceCard({
  ...
  return (
    <div className="rounded-3xl border border-white/[0.07] bg-white/[0.03] overflow-hidden transition hover:border-white/[0.12]">
      {/* Card header */}
      <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-zinc-600 flex items-center gap-1">
              <MapPin className="h-3 w-3" />{commerce.adresse}
            </p>
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          >
            {expanded ? <><ChevronUp className="h-3.5 w-3.5" />Réduire</> : <><ChevronDown className="h-3.5 w-3.5" />Détails</>}
          </button>
          ...
          {commerce.statut === "Approuve" && (
            <button
              ...
              onClick={() => onReject(commerce)}
              disabled={isLoading}
              className="flex items-center gap-1.5 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-1.5 text-xs font-semibold text-red-300 transition hover:bg-red-500/20 disabled:opacity-50"
            >
          ...
          {commerce.statut === "Rejete" && (
            <button
              ...
              onClick={() => onValidate(commerce.id)}
              disabled={isLoading}
              className="flex items-center gap-1.5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 text-xs font-semibold text-emerald-300 transition hover:bg-emerald-500/20 disabled:opacity-50"
            >
              <CheckCircle2 className="h-3.5 w-3.5" />
              {isLoading ? "…" : "Réapprouver"}
            </button>
          )}
        </div>
      </div>
        ...
        <div className="border-t border-white/[0.06] bg-black/20 px-5 py-5">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-1">
              <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-600 flex items-center gap-1">
                <User className="h-3 w-3" />Propriétaire ID
              </p>
              <p className="font-mono text-xs text-zinc-300 break-all">{commerce.proprietaireUtilisateurId}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-600 flex items-center gap-1">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-zinc-300">{formatDate(commerce.dateCreation)}</p>
            </div>
            <div className="space-y-1 sm:col-span-2 lg:col-span-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-600">Description</p>
              <p className="text-xs leading-5 text-zinc-400 whitespace-pre-line">{commerce.description}</p>
            </div>
            {commerce.tagsCulturels.length > 0 && (
              <div className="space-y-2 sm:col-span-2 lg:col-span-3">
                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-600 flex items-center gap-1">
                  <Tag className="h-3 w-3" />Tags culturels
                </p>
                <div className="flex flex-wrap gap-2">
                  {commerce.tagsCulturels.map(t => (
                    <span key={t} className="rounded-full border border-orange-500/25 bg-orange-500/10 px-2.5 py-1 text-[11px] font-medium text-orange-300">{t}</span>
                  ))}
                </div>
              </div>
            )}
            {commerce.statut === "Rejete" && commerce.raisonRejet && (
              <div className="sm:col-span-2 lg:col-span-3 rounded-2xl border border-red-500/20 bg-red-500/5 px-4 py-3">
                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-red-500/70">Raison du rejet</p>
                <p className="mt-1 text-xs text-red-300">{commerce.raisonRejet}</p>
              </div>
            )}
          </div>
        </div>
...
export default function AdminCommercesPage() {
  ...
  const filtered = commerces.filter(c => {
    ...
    const matchSearch = !q || c.nom.toLowerCase().includes(q) || c.adresse.toLowerCase().includes(q) || (c.nomCategorie ?? "").toLowerCase().includes(q);
  ...
  async function handleRejectConfirm(raison: string) {
    ...
    try {
      ...
      setCommerces(prev => prev.map(c => c.id === rejectTarget.id ? updated : c));
    ...
    <div className="space-y-6 text-white">
      ...
      <div className="rounded-3xl border border-white/[0.07] bg-gradient-to-br from-white/[0.04] to-transparent p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-amber-500/25 bg-amber-500/10 px-3 py-1 text-[11px] font-black uppercase tracking-[0.2em] text-amber-400">
              <Filter className="h-3 w-3" />Administration
            </div>
            <h1 className="text-2xl font-black">Gestion des commerces</h1>
            <p className="mt-1 text-sm text-zinc-500">
              Examinez, approuvez ou rejetez les demandes d'enregistrement.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "Total",       value: counts.tous,      color: "text-zinc-200",   bg: "bg-white/5"         },
            { label: "En attente",  value: counts.EnAttente,  color: "text-amber-300",  bg: "bg-amber-500/10"    },
            { label: "Approuvés",   value: counts.Approuve,  color: "text-emerald-300", bg: "bg-emerald-500/10" },
            { label: "Rejetés",     value: counts.Rejete,    color: "text-red-300",     bg: "bg-red-500/10"      },
          ].map(s => (
            <div key={s.label} className={`rounded-2xl border border-white/[0.07] ${s.bg} px-4 py-3`}>
              <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-zinc-600">{s.label}</p>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-1 rounded-2xl border border-white/[0.07] bg-white/[0.03] p-1">
          {TABS.map(t => (
            ...
            >
              {t.label}
              <span className={`rounded-full border border-white/10 px-1.5 py-0.5 text-[10px] font-black ${tab === t.key ? t.color : "text-zinc-600"}`}>
                {counts[t.key]}
              </span>
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="rounded-3xl border border-dashed border-white/[0.07] py-16 text-center">
          ...
          <p className="mt-4 text-sm font-semibold text-zinc-500">
            {search ? "Aucun résultat pour cette recherche." : "Aucun commerce dans cette catégorie."}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
