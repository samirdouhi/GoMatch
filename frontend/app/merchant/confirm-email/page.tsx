...
import { confirmProfessionalEmailRequest } from "@/lib/authApi";
...
function ConfirmProfessionalEmailContent() {
  ...
  const [message, setMessage] = useState(
    "Confirmation de votre email professionnel..."
  );
  ...
  useEffect(() => {
    ...
    async function runConfirmation() {
      ...
      try {
        const data = await confirmProfessionalEmailRequest(token);
        ...
        setMessage(
          data?.message ||
            "Email professionnel confirmé avec succès."
        );
      } catch (error: unknown) {
        if (!isMounted) return;
      ...
      <div className="relative z-10 w-full max-w-2xl overflow-hidden rounded-[32px] border border-white/10 bg-white/[0.04] shadow-[0_0_60px_rgba(0,0,0,0.45)] backdrop-blur-2xl">
        <div className="border-b border-white/10 bg-gradient-to-r from-yellow-400/10 via-orange-500/10 to-transparent px-8 py-7">
          ...
          <h1 className="mt-3 text-3xl font-black uppercase italic tracking-tight text-white md:text-4xl">
            Confirmation email professionnel
          </h1>
          <p className="mt-2 max-w-xl text-sm text-slate-400">
            Validation de votre adresse email professionnelle avant traitement
            de votre demande commerçant.
          </p>
        </div>
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                {isError && (
                  <p className="mt-3 text-sm text-slate-400">
                    Le lien peut être invalide, expiré ou déjà utilisé.
                  </p>
                )}
              </div>
            </div>
