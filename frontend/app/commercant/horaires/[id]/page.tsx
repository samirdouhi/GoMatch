...
import { ArrowLeft, Clock, Save, Trash2, CheckCircle2 } from "lucide-react";
...
export default function HorairesPage() {
  ...
  const [commerceNom, setCommerceNom] = useState<string>("");
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const [commerce, horaires] = await Promise.all([
          getCommerceById(commerceId),
          getHoraires(commerceId),
        ]);
  ...
  async function saveAll() {
    const dirtyJours = JOURS.filter((j) => rows[j.value]?.dirty);
    await Promise.all(dirtyJours.map((j) => saveRow(j.value)));
  ...
  return (
    <div className="space-y-6 text-white">
      {/* Retour */}
      <button type="button" onClick={() => router.push(`/commercant/${commerceId}`)}
        className="inline-flex items-center gap-2 text-sm font-semibold text-zinc-400 transition hover:text-white">
        <ArrowLeft className="h-4 w-4" />{commerceNom || "Retour au commerce"}
      </button>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            ...
            <span className="ml-1 text-zinc-600">00:00 = minuit (fermeture en fin de nuit)</span>
          </p>
        </div>
        <button
          type="button"
        ...
        >
          <Save className="h-4 w-4" />Tout sauvegarder
        </button>
      </div>
      ...
      {globalMsg && (
        <div className="flex items-center gap-2 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300">
          <CheckCircle2 className="h-4 w-4 shrink-0" />{globalMsg}
        </div>
      )}
      ...
      <p className="text-xs text-zinc-600">
        ...
        Pour fermer à minuit, entrez <strong className="text-zinc-400">00:00</strong>.
        Sauvegardez chaque ligne ou utilisez <strong className="text-zinc-400">Tout sauvegarder</strong>.
      </p>
    </div>
  );
}
